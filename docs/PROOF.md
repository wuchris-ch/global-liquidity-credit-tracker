...
| 10 | Backtest with **explicit weekly timing controls** and paired edge CIs | Friday weekly grid, next-bar entry, expanding-window classifier, and full-calendar paired moving-block bootstrap in [`src/indicators/backtest.py`](../src/indicators/backtest.py); the methodology separately discloses current-vintage reconstruction | `pytest tests/test_backtest.py` |
