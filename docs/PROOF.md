...
| 2 | **Scheduled Python pipeline** ingesting macro and market data | Clients in [`src/data_sources/`](../src/data_sources), including FRED, BIS, World Bank, NY Fed, Yahoo Finance, State Street, and OCC; orchestration in [`scripts/update_data.py`](../scripts/update_data.py) and [`src/indicators/flows.py`](../src/indicators/flows.py); inventory in [SOURCES.md](SOURCES.md) | `python cli.py list series` |
...
| 10 | Backtest with **explicit timing and multiplicity controls** | Friday weekly grid, next-bar entry, the production rolling GLCI classifier, an expanding NFCI benchmark, paired moving-block inference, and one Benjamini-Yekutieli adjustment over every finite displayed edge in [`src/indicators/backtest.py`](../src/indicators/backtest.py) | `pytest tests/test_backtest.py` |
| 11 | **Observed forward record** of published signals | The append-only ledger preserves repeated vintages; [`src/indicators/live_evaluation.py`](../src/indicators/live_evaluation.py) selects the first publication per signal date and evaluates only later weekly market bars | `pytest tests/test_storage_snapshots.py tests/test_live_evaluation.py` |
| 12 | Calculation correctness is **tested** | [`tests/`](../tests) - 200+ offline unit tests over published formulas and evidence controls; gated in CI before any publish ([`ci.yml`](../.github/workflows/ci.yml), [`update-data.yml`](../.github/workflows/update-data.yml)) | `make test` |
| 13 | **11-sector rotation, ETF issuance, and options activity** with separate evidence labels | Price-only rank, split-adjusted State Street net issuance, and non-directional OCC cleared activity in [`src/indicators/sector_rotation.py`](../src/indicators/sector_rotation.py); formulas and source boundaries in [MARKET_FLOWS.md](MARKET_FLOWS.md) | `pytest tests/test_sector_rotation.py` |
