...
| 10 | Backtest with **no look-ahead bias** and bootstrap CIs | Expanding-window classifier + moving block bootstrap in [`src/indicators/backtest.py`](../src/indicators/backtest.py); look-ahead invariance is a unit test | `pytest tests/test_backtest.py::TestExpandingZscoreRegime::test_no_lookahead_bias` |
