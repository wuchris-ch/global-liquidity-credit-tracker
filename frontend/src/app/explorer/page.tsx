...
export default function ExplorerPage() {
  ...
  // Auto-select first two series when list loads
  useEffect(() => {
    if (availableSeries.length > 0 && selectedSeries.length === 0) {
      const defaults = availableSeries.slice(0, 2).map(s => s.id);
      setSelectedSeries(defaults);
    }
  }, [availableSeries]);
  ...
  const chartData = useMemo(() => {
    ...
    // Find the series with the most data points to use as base
                ...
                <Card className="flex h-[500px] items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">
                      Select up to 5 series from the sidebar to compare
                    </p>
                  </div>
                </Card>
              ) : (
                <div className="space-y-6">
                  ...
                  {dataLoading ? (
                    ...
                    <MultiLineChart
                      ...
                      description={
                        normalize
                          ? "All series indexed to 100 at start of period"
                          : "Absolute values (different scales)"
                      }
                      data={chartData}
                      series={chartSeries}
                      height={450}
                              ...
                              return (
                                <tr key={id} className="border-b border-border/50">
                                  ...
                                  <td className="py-3 text-right font-mono">
                                    {dataLoading ? "..." : latest.toLocaleString(undefined, {
                                      maximumFractionDigits: 2,
                                    })}
                                  </td>
                                  <td className="py-3 text-right font-mono text-muted-foreground">
                                    {dataLoading ? "..." : min.toLocaleString(undefined, {
                                      maximumFractionDigits: 2,
                                    })}
                                  </td>
                                  <td className="py-3 text-right font-mono text-muted-foreground">
                                    {dataLoading ? "..." : max.toLocaleString(undefined, {
                                      maximumFractionDigits: 2,
                                    })}
                                  </td>
                                  <td
