...
interface GlciChartProps {
  data: DataPoint[];
  periods?: RegimePeriod[];
  height?: number;
}

/** GLCI line on paper, with regime washes behind the periods in force. */
export function GlciChart({ data, periods = [], height = 320 }: GlciChartProps) {
  // Clamp regime periods to dates that exist in the visible data so the
  // category x-axis can place the reference areas.
  const bands = useMemo(() => {
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
          ...
          <Tooltip
            ...
            formatter={(value) => [Number(value).toFixed(1), "GLCI"]}
          />
          <Line
            type="monotone"
            dataKey="value"
