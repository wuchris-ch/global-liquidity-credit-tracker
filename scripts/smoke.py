...
def check_local_artifacts(s: Smoke) -> None:
    ...
    # The trust endpoint was introduced with the current export contract, so
    # older generated artifacts should be treated as absent rather than as a
    # failed run of the current pipeline.
    probe = export_dir / "api" / "glci" / "trust" / "index.json"
    if not probe.is_file():
        s.warn(
            "local export",
            "current export not present (expected: data/ is generated by the pipeline)",
        )
        return
